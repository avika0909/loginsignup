...
document.getElementById('verifyButton').addEventListener('click', async function() {
    ...
    try {
        const response = await fetch('https://backend-appp-d3eshpcacmasccd3.centralindia-01.azurewebsites.net/validate', {
            method: 'POST',
            headers: {
...
document.getElementById('forgot-password-form').addEventListener('submit', async function(event) {
    ...
    if (newPassword && confirmPassword) {
        if (newPassword === confirmPassword) {
            try {
                const response = await fetch('https://backend-appp-d3eshpcacmasccd3.centralindia-01.azurewebsites.net/reset', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
